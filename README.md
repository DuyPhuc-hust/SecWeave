...
Chưa có: redaction policy đầy đủ theo danh mục thật (§4.3.5, cần chốt cùng owner ở Gate 3 — hiện chỉ có floor tối thiểu), manifest cấp package (§4.3.3), Verification Package assembler (§7, 19 trường — chỉ có schema mô tả, chưa có code build ra object thật). Human Review Loop: chưa hiện thực. Kill-switch nối CLI/API thật: chưa hiện thực (xem 2 lớp hạn chế ở trên).
