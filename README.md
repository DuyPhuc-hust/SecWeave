...
Chưa có: redaction policy đầy đủ theo danh mục thật (§4.3.5, cần chốt cùng owner ở Gate 3 — hiện chỉ có floor tối thiểu), manifest cấp package (§4.3.3). Human Review Loop: chưa hiện thực (có sẵn model `HumanReviewRecord`/`ReviewDecision` nhưng chưa có workflow/CLI thật). `execute` chưa tự dựng được kịch bản 3-role (main/positive_control/denied_control) — vẫn cần script tay.
